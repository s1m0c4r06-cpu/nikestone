...
if (multiForm) {
  multiForm.addEventListener("submit", async e => {
    ...
    // Sync _replyto with the email field so Formspree sends the autoresponder to the lead
    const emailVal = document.getElementById("email")?.value || "";
    const replytoField = document.getElementById("replytoField");
    if (replytoField) replytoField.value = emailVal;

    // Collect values for structured owner email
    ...
    const ambitiChecked = Array.from(document.querySelectorAll("input[name='ambito']:checked")).map(c => c.value);

    const strutturato = [
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      "NUOVO LEAD — NIKESTONE.IT",
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      ...
      "──────────────────────────────",
      `IMMOBILE   : ${immobile}`,
      `AMBITI     : ${ambitiChecked.join(", ") || "—"}`,
      `FASE       : ${fase}`,
      `NOTE       : ${desc || "—"}`,
      `ALLEGATI   : ${uploadedFiles.map(f => f.name).join(", ") || "Nessuno"}`,
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ].join("\n");

    const formData = new FormData(multiForm);
    formData.set("message", strutturato);
