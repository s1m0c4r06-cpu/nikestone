...
function initSteppedForm(){
  ...
  function validateStep(n){
    ...
    if(!ok){
      showStepMsg(n, 'Compila i campi obbligatori evidenziati per continuare.');
    } else {
      clearStepMsg(n);
    }
    return ok;
  ...
  function stepMsgBox(n){
    const step = form.querySelector('.fstep[data-step="' + n + '"]');
    if(!step) return null;
    let box = step.querySelector('.step-err');
    if(!box){
  ...
  function showStepMsg(n, text){ const b = stepMsgBox(n); if(b) b.textContent = text; }
  function clearStepMsg(n){ const b = stepMsgBox(n); if(b) b.textContent = ''; }
  ...
  // file display — label already opens the native picker (no extra .click() to avoid double dialog)
  if(fileInput && fileDisplay){
    fileDisplay.addEventListener('click', () => fileInput.click());
    fileInput.addEventListener('change', () => {
      const f = fileInput.files[0];
      if(!f){ fileDisplay.textContent = '📎 Scegli file…'; return; }
  ...
  form.addEventListener('submit', async (e) => {
    e.preventDefault();
    for(const s of [1,2,3]){ if(!validateStep(s)){ setStep(s); return; } }
    ...
    try {
      ...
      // invio a FormSubmit -> email a massimoangelino23@gmail.com (no-cors: risposta opaca)
      await fetch(form.action, { method: 'POST', mode: 'no-cors', body: fd });
      showMsg('✓ Richiesta inviata! Ti ricontattiamo entro 48 ore lavorative. Grazie.', 'ok');
      form.reset();
      if(fileDisplay){ fileDisplay.textContent = '📎 Scegli file…'; fileDisplay.style.color = ''; }
      form.querySelectorAll('.is-checked').forEach(w => w.classList.remove('is-checked'));
      [1,2,3].forEach(s => clearStepMsg(s));
    } catch(err){
      showMsg('Invio non riuscito. Scrivici a massimoangelino23@gmail.com o chiamaci al +39 331 638 1673.', 'err');
    } finally {
      btn.disabled = false;
      btn.textContent = ot;
    }
  ...
  inputs.forEach(input => {
    ...
    if(wrap){
      ...
      wrap.addEventListener('click', e => {
        // direct click on the input itself: let the browser toggle it natively
        if(e.target === input) return;
        // click anywhere else in the label: drive the input ourselves so the
        // selection is guaranteed even if native label->input toggling fails
        e.preventDefault();
        if(input.type === 'radio'){
...
const TRANSLATIONS = {
  it: {
    ...
    'hero.sub':'Lavoriamo e posiamo Pietra di Luserna per chalet di lusso, ville private e progetti d\'autore. Quarant\'anni di mestiere tra la Francia e la Svizzera.',
    'hero.cta1':'Vedi i lavori','hero.cta2':'Inizia il tuo progetto',
    'about.kicker':'Chi siamo',
    'about.h':'Una tradizione di famiglia, una nuova generazione.',
    'about.p1':'Nikestone nasce dalle mani di Mauro Angelino, che a fine carriera ha lasciato il compito di guidare l\'attività al figlio Massimo Angelino: quattro decenni di pietra lavorata e posata in cantieri tra Francia, Germania, Svizzera e Italia.',
    'about.p2':'Lavoriamo per architetti, interior designer, general contractor e committenti privati che vogliono superfici autentiche e risultati garantiti. Ogni progetto è unico, un dialogo costante tra committente e azienda.',
    ...
    'services.h':'Quello che facciamo, eseguito a regola d\'arte.',
  ...
  fr: {
    ...
    'hero.sub':'Nous taillons et posons la Pierre de Luserne pour chalets de prestige, villas privées et projets d\'auteur. Quarante ans de savoir-faire entre la France et la Suisse.',
    'hero.cta1':'Voir les réalisations','hero.cta2':'Commencer votre projet',
    'about.kicker':'À propos',
    'about.h':'Une tradition de famille, une nouvelle génération.',
    'about.p1':'Nikestone est né des mains de Mauro Angelino qui, en fin de carrière, a confié la conduite de l\'entreprise à son fils Massimo Angelino : quatre décennies de pierre travaillée et posée sur des chantiers entre la France, l\'Allemagne, la Suisse et l\'Italie.',
    'about.p2':'Nous travaillons pour les architectes, designers d\'intérieur, general contractors et maîtres d\'ouvrage privés qui veulent des surfaces authentiques et des résultats garantis. Chaque projet est unique, un dialogue constant entre le client et l\'entreprise.',
    'about.sign':'— Massimo & Mauro Angelino',
